...
API; this module only supplies live status data.
"""
...
class RouterWebSocketMonitor:
    ...
    def _connection_info(self) -> tuple[str, str, str, bool]:
        ...
        return ws_url, origin, cookie, bool(cfg.get("verifyTls", False))
    ...
    def _run_connection(self, ws_url: str, origin: str, cookie: str, verify_tls: bool) -> None:
        sslopt = None
    ...
    def _loop(self) -> None:
        ...
        last_logged_error = ""
        while not self._stop.is_set():
            ws_url, origin, cookie, verify_tls = self._connection_info()
            if not ws_url:
                self._stop.wait(2.0)
                continue
            try:
                self._run_connection(ws_url, origin, cookie, verify_tls)
                retry = 1.0
                last_logged_error = ""
...
def _dashboard(self: Any, force: bool = False) -> Dict[str, Any]:
    result = _ORIGINAL_DASHBOARD(self, force)
    ...
    output["wsStatus"] = snapshot.get("wsStatus", {})
    return output
...
def _normalize_dashboard(self: Any, raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for row in radios:
        ...
        use = _first_text(row, "channel_utilization", "channelUtilization", "chutil", "utilization")
        if band:
            bands.append(band)
        if channel:
    ...
    normalized["online"] = bool(slow.get("connected")) or _truthy(slow.get("status")) or bool(_dict(raw.get("wsStatus")).get("connected"))
