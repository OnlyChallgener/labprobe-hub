...
patch_path = Path('stun_port_config_patch.py')
patch_path.write_text('''"""STUN middle-port policy without changing Router Core or realtime paths.
...
# Version bump makes the router binary containing this STUN behavior distinct
# from previous 0.2.38 bundles.
cargo = Path('labrelay/Cargo.toml')
text = cargo.read_text(encoding='utf-8')
assert 'version = "0.2.38"' in text
cargo.write_text(text.replace('version = "0.2.38"', 'version = "0.2.39"', 1), encoding='utf-8')

lock_path = Path('labrelay/Cargo.lock')
lock = lock_path.read_text(encoding='utf-8')
pattern = r'(\[\[package\]\]\nname = "labrelay"\nversion = ")0\.2\.38(")'
lock, count = re.subn(pattern, r'\g<1>0.2.39\2', lock, count=1)
assert count == 1, 'LabRelay Cargo.lock version entry missing'
lock_path.write_text(lock, encoding='utf-8')

ci_path = Path('.github/workflows/ci.yml')
ci = ci_path.read_text(encoding='utf-8')
assert 'grep -q \'version = "0.2.38"\' labrelay/Cargo.toml' in ci
ci = ci.replace('grep -q \'version = "0.2.38"\' labrelay/Cargo.toml', 'grep -q \'version = "0.2.39"\' labrelay/Cargo.toml', 1)
ci_path.write_text(ci, encoding='utf-8')

# Behavioral tests use a small fake STUN service so the policy is verified
# without touching Router Core or the live router driver.
test_path = Path('tests/test_stun_middle_port_wiring.py')
...
'''
if 'test_legacy_stun_middle_port_is_migrated_out_of_20000_pool' not in test:
    test += append
